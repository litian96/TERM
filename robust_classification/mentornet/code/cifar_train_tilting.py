...
def train_inception_baseline(max_step_run):
  ...
  with g.as_default():
    ...
    with tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks)):
      ...
      tt = tf.constant(FLAGS.tilting, dtype=tf.float32)
      tilt = (tt / max_step_run) * (tf.cast(tf_global_step, tf.float32)+1)
      total_loss = (1.0 / tilt) * tf.reduce_mean(tf.exp(tilt * loss))
