...
def train_inception_baseline(max_step_run):
  ...
  with g.as_default():
    ...
    with tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks)):
      ...
      # reweight by the rectified gaussian distribution
      loss = tf.reshape(total_loss, [-1, 1])
      z = tf.reshape(tf.random_normal((1,FLAGS.batch_size)), [-1, 1])
      z = tf.math.maximum(z, 0)
      weights = z / tf.reduce_sum(z)
      total_loss = tf.reduce_sum(weights * loss)
