...
from mnist.reweight_focal import get_model
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        with tf.name_scope('Train'):
            _, loss_c, logits_c = get_model(
                x_, y_, 0.05, 2, is_training=True, dtype=tf.float32, w_dict=None, reuse=None)
        ...
        with tf.name_scope('Val'):
            _, loss_eval, logits_eval = get_model(
                x_,
                y_,
                0.05,   # alpha for focal loss
                2,      # gamma for focal loss
                is_training=False,
                dtype=tf.float32,
                w_dict=None,
                reuse=True)
