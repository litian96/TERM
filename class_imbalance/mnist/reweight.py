...
def get_model(inputs,
    ...
    with tf.variable_scope('Model', reuse=reuse):
        ...
        individual_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=logits, labels=labels)
    ...
    return w_dict, loss, logits, individual_loss
...
def reweight_autodiff(inp_a,
    ...
    w_dict, loss_a, logits_a, _ = get_model(
...
def reweight_tilting(inp_a,
                     label_a,
                     bsize_a,
                     gate_gradients=1):

    w_dict, loss_a, logits_a, individual_loss = get_model(
        inp_a, label_a, is_tilting=True, ex_wts=None, is_training=True, reuse=True)

    loss_sorted, loss_sort_idx = tf.nn.top_k(2000 * individual_loss + sample_gumbel(tf.shape(loss_a)), k=100)
    #ex_weight = tf.zeros([bsize_a], dtype=tf.float32)

    return loss_sort_idx
...
def reweight_hard_mining(inp, label, positive=False):
    ...
    """
    _, loss, logits, _ = get_model(inp, label, ex_wts=None, is_training=True, reuse=True)
